...
private:
	...
	value_type node;
public:
	Node(value_type _node = value_type())
	{
		node = _node;
	}
	string getObject()
...
public:
	...
	void addNode(T _nodeObject)
	{
		...
		{
			node.push_back(Node<value_type>(_nodeObject));
		}
	};
	...
	void addEdge(value_type iObject, value_type jObject)
	{
	...
	short nodeDegree(value_type _nodeObject)
	{
		short degree = 0;
	...
	void deleteEdge(value_type iObject, value_type jObject)
	{
		for (int k = 0; k < edge.size(); k++)
		{
			if (isEdgeExist(iObject, jObject) != -1)
			{
				edge.erase(edge.begin() + isEdgeExist(iObject, jObject));
				break;
			}
		}
	};
	...
	short isEdgeExist(value_type iObject, value_type jObject)
	{
		for (int k = 0; k < edge.size(); k++)
		{
	...
	short isNodeExist(value_type _nodeObject)
	{
		for (int i = 0; i < node.size(); i++)
		{
	...
	private:
		typedef T value_type;
		Graph<value_type>* graphForIteration;
		int index;
	public:
		Node_Iterator(Graph<value_type>& graph)
		{
	...
	public:
		Edge_iterator(Graph<value_type>& graph)
		{
			graphForIteration = &graph;
			index = 0;
		}
